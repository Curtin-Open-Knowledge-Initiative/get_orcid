...
async function fetch_many(ids=[]) {
  ...
  const records = [];
  for (let id of ids) {
    const record = await fetch_one(id);
...
async function fetch_one(str='0000-0000-0000-0000') {
  const id = clean_orcid(str);
  if (!id) {
    console.log('Invalid ORCID value',str);
    return null;
  }
...
function clean_orcid(str='') {
  const orcid = str.trim().toUpperCase().split(/[^0-9X]/).join('-');
  return /^[0-9]{4}\-[0-9]{4}\-[0-9]{4}\-[0-9]{3}[0-9X]$/.test(orcid) ? orcid : '';
}
