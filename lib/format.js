...
function push(idir='data',cloud='your-bucket') {
  execSync(`gsutil cp ${resolve(idir)}/works* gs://${cloud}/`);
}
...
function flatten(records=[]) {
  ...
  for (let record of records) {
    for (let work of (record.pubs.length ? record.pubs : [{}])) {
      flat.push({
        orcid   : record.orcid,
        found   : record.found,
    ...
    <html>
      ...
      <body>
        ...
        ${sort_names(data).map(person => `
          <details>
            <summary>
${person.name.family_name}, ${person.name.given_names} (<a href='https://orcid.org/${person.orcid}'>ORCID</a>)
            </summary>
            <div>
              <div>${person.pubs.length} works listed in ORCID</div>
              <ul>
${sort_works(person.pubs).map(work => `<li>${work.date} - ${work.link ? `<a href='${work.link}'>${work.title}</a>` : work.title} [${work.type || 'work'}]</li>`).join("\n")}
              </ul>
            </div>
          </details>
        `).join("\n")}
...
function write_html_sortable(data=[],ofile='data/publications_sortable.html') {
  const table = flatten(data).map(v => ({
    name  : `<a href='https://orcid.org/${v.orcid}'>${v.family}, ${v.given}</a>`,
    year  : v.year,
